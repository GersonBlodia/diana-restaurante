 const distritos = await prisma.distrito.findMany();
  ...
  for (const emp of empleadosRaw) {
    const persona = emp.dni
      ...
      : personas.find(
          (p) =>
            ...
            p.apellido.trim().toLowerCase() ===
              emp.apellido?.trim().toLowerCase()
        );

    if (!persona) {
      console.warn(
        `❌ Persona no encontrada para el empleado ${
          emp.dni ?? emp.nombre + " " + emp.apellido
        }`
      );
      continue;
    }
    ...
    try {
      ...
      console.error(
        `⚠️ Error al crear empleado para persona ${persona.nombre} ${persona.apellido}:`,
        error
      );
    }
  }
